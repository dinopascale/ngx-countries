...
export class CountriesApiService {
    ...
    private withFilterFactory = (url: string) => <D extends keyof Country>({filters, param}: {filters?: D[], param?: string}) => this.http.get<Pick<Country, keyof Country>[]>(addFilters(addDynamicParams(this.baseCountryApiUrl + url, param), filters));

    constructor(private http: HttpClient) {}

    getAllCountries = this.withFilterFactory('/all');

    getCountryByName = this.withFilterFactory('/name');
